...
const oppId = nextMatch ? (nextMatch.strAwayTeam === oppName ? nextMatch.idAwayTeam : nextMatch.idHomeTeam) : null;
let oppLastEvents = [];
if (oppId && API_KEY) {
...
const fill = (template, vars) => Object.entries(vars).reduce((s, [k, v]) => s.replaceAll(`{{${k}}}`, v ?? '—'), template);
...
fs.writeFileSync(path.join(out, 'data.json'), JSON.stringify({ teamName, oppName, teamRow, oppRow, nextMatch, nextMatchIso, nextEvents, ligue2Ready, lastEvents, oppLastEvents, formFCSM, formOpp }, null, 2), 'utf8');
...
console.log('dist/ generated', { teamName, oppName, formFCSM, formOpp, ligue2Ready, nextMatchIso, leagueEvents: leagueEvents.length, teamLigue2Events: teamLigue2Events.length, totalEvents: (nextTeamData?.events || []).length });
